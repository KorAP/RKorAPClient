...
setMethod("corpusStats", "KorAPConnection",  function(kco, vc="", verbose = kco@verbose, as.df = FALSE) {
  ifelse(length(vc) > 1,
    return(
      sapply(vc, function(cq) corpusStats(kco, cq, verbose, as.df = TRUE)) %>%
        apply(1, unlist) %>%
        as.data.frame() %>%
        rownames_to_column("vc")
    ), {
    url <- paste0(kco@apiUrl, 'statistics?cq=', URLencode(vc, reserved=TRUE))
    log.info(verbose, "Calculating size of corpus \"", vc,"\"", sep="")
    res <- apiCall(kco, url)
    log.info(verbose, "\n")
    ifelse(as.df, return(as.data.frame(vc=vc, res)),
      return(new("KorAPCorpusStats", vc = vc, documents = res$documents, tokens = res$tokens, sentences = res$sentences, paragraphs = res$paragraphs))
    )
  })
})
