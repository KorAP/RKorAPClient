            ...
            if (addExamples && length(result) > 0) {
              ...
              result$example = findExample(
                ...
                query = buildCollocationQuery(
                  removeWithinSpan(result$node, withinSpan),
                  result$collocate,
                  leftContextSize = leftContextSize,
                  rightContextSize = rightContextSize,
                  withinSpan = withinSpan
...
findExample <-
  function(kco,
    ...
    for(i in seq_along(query)) {
      ...
      example <- (q@collectedMatches)$snippet[1]
      if (is.null(example))
        out[i] = ""
      else
        if(matchOnly) {
          if (str_detect(example, '.*<mark>(.+)</mark>.*'))
            out[i] <- str_replace(example, '.*<mark>(.+)</mark>.*', '\\1')
          else
            out[i] <- example
        } else {
          out[i] <- str_replace(example, '<[^>]*>', '')
        }
    }
    out
  }
