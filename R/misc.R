...
#' @param ...	 Other arguments passed to geom_ribbon, geom_line, amnd geom_click_point.
#'
#' @examples
#' library(ggplot2)
...
tooltip2hyperlink <- function(p, attribute="webUIRequestUrl") {
  ...
  for(i in grep(attribute, p$x$data)) {
    x <- p[["x"]][["data"]][[i]][["text"]]
    m <- regexpr(pattern, x)
    matches <- sub(paste0(attribute, "webUIRequestUrl: "), "", regmatches(x, m))
...
#'
#' This function converts a \code{ggplot2::ggplot()} object to a plotly
#' objects with hyperlinks from data points to corresponding KorAP queries.
#'
...
#' @export
ggplotly <- function(p = ggplot2::last_plot(), tooltip = c("x", "y", "url"), ...) {
