...
hc <- highchart() %>%
  ...
  hc_xAxis(categories = topics %>% str_to_title(locale = "en") )%>%
  hc_yAxis(type = "logarithmic") %>%
  hc_legend(enabled=FALSE) %>%
  hc_title(text="Thema")
hc
...
texttype_vc <- sprintf("%stextType=%s", vc, texttypes)
...
hc <- highchart() %>%
  hc_add_series(type = "bar", data = df, hcaes(x=vc, tokens), name="Tokens") %>%
  hc_xAxis(categories = texttypes %>% str_replace_all("[/.*)()]", "") %>% str_replace_all("\\|", "/")) %>%
  hc_yAxis(type = "logarithmic") %>%
  hc_legend(enabled=FALSE) %>%
