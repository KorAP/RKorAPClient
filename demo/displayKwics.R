...
if (is.null(kco@accessToken)) {
  stop("To receive KWICSs from the API and to run this demo, you need an access token which you can currently only get as a beta tester. If you have an access token, you also need to persist it with 'persistAccessToken()'.")
}
